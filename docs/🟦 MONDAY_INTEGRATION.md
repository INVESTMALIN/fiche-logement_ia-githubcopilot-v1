...
> **BAC secours (2026-09-25)** : colonne status créée le 25/09, juste après « Boîte à clé ». Valeur dérivée côté front (`valeurMondayBacSecours`, `src/lib/clefsSecours.js`) : `TTlock` (index 0) ou `Masterlock` (index 1) si la fiche répond « oui » à la boîte à clés de secours, sinon vide (`{}`). Index lus dans `settings_str` le 25/09 (`BAC_SECOURS_INDEX`). La colonne « Boîte à clé » de la boîte **principale** reste remplie **à la main** par les coordinateurs : aucun code n'y touche. Les snapshots antérieurs n'ont pas la clé `bac_secours` → un push de vide vers une colonne vide à la prochaine sauvegarde des fiches Complété, inoffensif. Aucun rattrapage (champ nouveau).
